        ...
        #  self.getUrl(url1,url2,url3,search,page)
            ...
            if h3_soup:
                ...
                for h3 in h3_soup:
                    # 用replace去除不相关的内容
                    href_num += 1
                    print(h3.get_text().replace('中文图书','').replace('西文图书','').replace('规范文档',''))
...
# try:
    ...
    # search = input()
    # page = 1
    # url1 = 'http://210.29.99.7:8080/opac/openlink.php?dept=ALL&title='
    # url2 = '&doctype=ALL&lang_code=ALL&match_flag=forward&displaypg=20'
    # url3 = '&showmode=list&orderby=DESC&sort=CATA_DATE&onlylendable=no&count=482&with_ebook=&page='
    # url = url1 + str(search) + url2 + url3 + str(page)
    # # 伪装成浏览器
    ...
    # # a_soup = soup.find_all('ol',{'id':'search_book_list'})
    # # for a in a_soup.h3:
    ...
    # for h3 in h3_soup[0:20]:
        # print(h3.get_text().replace('中文图书',''))

    # num = int(input(u"请选择书籍"))

    # href = h3_soup[num-1].a.get('href')
    # url = 'http://210.29.99.7:8080/opac/' + href
    # request = urllib.request.Request(url, headers=headers)
    # response = urllib.request.urlopen(url)
    # content = response.read().decode('utf-8')
